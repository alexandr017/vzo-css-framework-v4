...
Array.from($$('.btn-toggle')).forEach(btn => btn.onclick = removeForm);

function removeFormBack(el) {
    el.preventDefault();
    let to_remove = this.parentNode.nextElementSibling;
    if ( formTemplate.style.display == 'block' ) {
        ...
        formTemplate.style.display = 'block';
        el.target.parentNode.insertAdjacentElement('afterend', formTemplate);
    }

}
Array.from($$('.r-btn-wrap')).forEach(btn => btn.onclick = removeFormBack);
