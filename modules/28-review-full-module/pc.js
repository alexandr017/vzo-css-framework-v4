...
let formTemplate = $$('#review-form')[0];
let backForm = $$('#review-f-bl')[0];
let bodyForm = $$('#form-1')[0];
let plus = $$('#plus')[0];
let minus = $$('#minus')[0];
let name = $$('#name')[0];
...
for(let i=0;i<moveBtnBlock.length;i++) {
    moveBtnBlock[i].addEventListener('click',function (e) {
        ...
        moveBtnBlock[i].nextElementSibling.style.display = 'block';
        if (to_remove == null) {
            formTemplate.style.display = 'block';
            plus.style.display = 'none';
            minus.style.display = 'none';
            name.classList.remove('review-form-style');
            moveBtnBlock[i].nextElementSibling.insertAdjacentElement('afterend', formTemplate);
            mainFormBlock.style.display = 'none';
            btnFormBlock.innerHTML = '<div class="r-btn-wrap flex">\n' +
                '<div class="cancel-btn" onclick="removeFormBack()">Отмена</div>\n' +
